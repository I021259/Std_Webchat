...
const decodeSso2_1 = base64.decode(mySapSso2_1),
  decodeSso2_2 = base64.decode(mySapSso2_2),
  decodeFukudaSso = base64.decode(fukudaSso)

const mySsoId_1 = decodeSso2_1.slice(8, 21).replace(/\0/g, ''),
...
const ssoUserId = Cookies.get(ssoCookieID)
//  ? base64.decode(Cookies.get(ssoCookieID)).slice(15, 22)
//  : 'Guest'
  ...
  getChannelPreferences(channelId, token).then(preferences => {
    ReactDOM.render(
      <Provider store={store}>
        <App
          token={token} channelId={channelId} preferences={preferences}
          ssoUserId={ssoUserId} />
      </Provider>,
      root,
    )
